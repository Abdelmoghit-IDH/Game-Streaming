...
function Results() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const searchResult = await api.get(`https://api.twitch.tv/helix/users?login=${cleanSearch}`);
      // console.log(searchResult);

      searchResult.data.data.length === 0 ? setResult(false) : setStreamerInfo(searchResult.data.data);
    })();
  }, [cleanSearch]);
  return result ? (
    <>
        ...
        {streamerInfo.map((stream, index) => (
          <div key={index} className="cardResults">
            ...
            <div className="cardBodyResults">
              ...
              >
                <div className="btnCard btnResult">Watch {stream.display_name}</div>
              </Link>
            </div>
          </div>
        ))}
